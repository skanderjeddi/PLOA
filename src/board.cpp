...
template <class T>
Board<T>::Board(const BoardProperties &properties) : properties(properties)
{
    this->tiles = std::map<std::pair<int, int>, T>();
}

template <class T>
Board<T>::Board(const Board &board) : properties(board.properties)
{
    this->tiles = board.tiles;
}

template <class T>
void Board<T>::setTile(int x, int y,
                       const T &tile)
{
...
template <class T>
std::vector<std::pair<TileEdge, T>> Board<T>::getNeighbors(const std::pair<int, int> &pos) const
{
    auto positions = std::vector<std::pair<int, int>>({std::pair<int,
                                                                 int>(pos.first - 1, pos.second),
                                                       std::pair<int,
                                                                 int>(pos.first, pos.second - 1),
                                                       std::pair<int,
                                                                 int>(pos.first + 1, pos.second),
                                                       std::pair<int,
                                                                 int>(pos.first, pos.second + 1)});
