...
public:
     ...
     */
    virtual int handleTile(const T &,
     ...
     */
    std::vector<std::pair<TileEdge,
                          T>>
    getNeighbors(const std::pair<int, int> &) const;
    /**
     ...
     */
    virtual bool canSet(const T &,
                        const std::pair<int, int> &) const = 0;
    /**
     * @brief Get the Properties object of the board.
     *
     * @return BoardProperties
     */
    BoardProperties getProperties() const
    {
        return properties;
    }
};